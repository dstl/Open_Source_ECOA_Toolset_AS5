...
public class DynTrigInstanceILIWriterC extends SourceFileWriter {
	...
	public DynTrigInstanceILIWriterC(Path outputDir, SM_DynamicTriggerInstance trigInst) {
		super(outputDir);
	...
	@Override
	public void close() {
		String closeText = "#endif  /* _"
				/*
				 * + trigInst. getComponentImplementation().
				 * getName().toUpperCase() + "_"
				 */ + trigInst.getName().toUpperCase() + "ILI_H */" + LF + LF;
	...
	@Override
	public void open() {
		super.openFile(outputDir.resolve(
				/* trigInst.getComponentImplementation().getName() + "_" + */ trigInst.getName() + "_ILI.h"));
	}
		...
		for (ILIMessage iliMessage : iliMessageList) {
			if (iliMessage instanceof EventILIMessage) {
				...
				for (SM_SenderInterface senderInterface : eventILI.getEventLink().getSenders()) {
					if (senderInterface.getSenderInst() instanceof SM_DynamicTriggerInstance) {
			...
			messageDefText += "#define "
					+ /*
						 * trigInst.getComponentImplementation().getName() + "_"
						 * +
						 */ trigInst.getName() + SEP_PATTERN_01 + iliMessage.getMessageID() + " " + iliMessage.getMessageID() + LF + LF;
		}
	...
	public void writeMessageStructure() {
		...
		for (ILIMessage iliMessage : iliMessageList) {
			if (iliMessage.getParams().size() > 0) {
				...
				messageStructText += "} "
						+ /*
							 * trigInst.getComponentImplementation().getName() +
							 * "_" +
							 */ trigInst.getName() + SEP_PATTERN_01 + iliMessage.getMessageID() + "_params;" + LF;
				...
				messageStructText += "/* "
						+ /*
							 * trigInst.getComponentImplementation().getName() +
							 * "_" +
							 */ trigInst.getName() + SEP_PATTERN_01 + iliMessage.getMessageID() + " has no parameters */" + LF;
			}
		}
	...
	public void writePreamble() {
		String preambleText = "/* File "
				+ /* trigInst.getComponentImplementation().getName() + "_" + */ trigInst.getName() + "_ILI.h */" + LF + LF +

				"#if !defined(_" + /*
									 * trigInst.getComponentImplementation().
									 * getName().toUpperCase() + "_" +
									 */ trigInst.getName().toUpperCase() + "ILI_H)" + LF + "#define _"
				+ /*
					 * trigInst.getComponentImplementation().getName().
					 * toUpperCase () + "_" +
					 */ trigInst.getName().toUpperCase() + "ILI_H" + LF + LF;
