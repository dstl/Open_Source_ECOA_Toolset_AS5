 ...
 * </ul>
 */
public class CompDefEditor extends MultiPageEditorPart implements IResourceChangeListener {
	...
	private CompDefUtil util = new CompDefUtil();

	/**
	 * Creates a multi-page editor example.
	 */
	public CompDefEditor() {
		super();
				...
				public void widgetSelected(SelectionEvent evt) {
					if (evt.item.getClass().getName().equalsIgnoreCase(TreeItem.class.getName())) {
						...
						if (parent != null) {
							...
							button.setText("Remove Def");
							button.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_CENTER | GridData.GRAB_HORIZONTAL | GridData.GRAB_VERTICAL));
							button.addMouseListener(new SelectMouseListener(sel, content, containerName));
						}
						composite.layout();
						if (parent != null && parent.getText().equalsIgnoreCase("Required Service")) {
							try {
								ComponentType data = util.getSelectedRef(sel.getText(), ComponentTypeReference.class);
								CompDefReferenceComposite comp = new CompDefReferenceComposite();
								comp.setDef((ComponentTypeReference) data.getServiceOrReferenceOrProperty().get(0));
								...
								button.setText("Save");
								button.addMouseListener(new SaveMouseListener(true, comp.getDef().getName(), comp, parent.getText()));
								dataDet.layout();
								composite.layout();
						...
						} else if (parent != null && parent.getText().equalsIgnoreCase("Provided Service")) {
							try {
								ComponentType data = util.getSelectedRef(sel.getText(), Service.class);
								CompDefServiceComposite comp = new CompDefServiceComposite();
								comp.setDef((Service) data.getServiceOrReferenceOrProperty().get(0));
								comp.setContainerName(containerName);
								comp.createPartControl(composite);
								Composite dataDet = comp.getRet();
								dataDet.setLayout(compLayout);
						...
						} else if (parent != null && parent.getText().equalsIgnoreCase("Properties")) {
							try {
								ComponentType data = util.getSelectedRef(sel.getText(), Service.class);
								CompDefPropertyComposite comp = new CompDefPropertyComposite();
								comp.setDef((Property) data.getServiceOrReferenceOrProperty().get(0));
								comp.setContainerName(containerName);
								comp.createPartControl(composite);
								Composite dataDet = comp.getRet();
								dataDet.setLayout(compLayout);
				...
				protected void disposeRemaining(Composite composite, TreeItem parent) {
					...
					for (Control ctrl : ctrls) {
						if (!(ctrl instanceof Tree)) {
							if (parent != null && parent.getText().equalsIgnoreCase("Definitions")) {
	...
	public class SelectMouseListener implements MouseListener {
		...
		public void mouseUp(MouseEvent e) {
			if (e.getSource() instanceof Button) {
				...
				if (btn.getText().equalsIgnoreCase("Add Def")) {
					GridLayout compLayout = new GridLayout();
					compLayout.numColumns = 2;
					GridData gd = new GridData(GridData.FILL_BOTH | GridData.GRAB_HORIZONTAL | GridData.GRAB_VERTICAL);
					...
					if (!level2Item(selItem)) {
						...
						if (selItem != null && selItem.getText().equalsIgnoreCase("Required Service")) {
							ComponentTypeReference data = new ComponentTypeReference();
							CompDefReferenceComposite comp = new CompDefReferenceComposite();
							comp.setDef(data);
							comp.setContainerName(containerName);
							comp.createPartControl(composite);
							child = comp.getRet();
							com = comp;
						} else if (selItem != null && selItem.getText().equalsIgnoreCase("Provided Service")) {
							...
							child = comp.getRet();
							com = comp;
						} else if (selItem != null && selItem.getText().equalsIgnoreCase("Properties")) {
				...
				} else if (btn.getText().equalsIgnoreCase("Remove Def")) {
					if (selItem == null || selItem.getParent() == null || selItem.getParent().getParent() == null) {
					} else {
						try {
							String tempText = util.removeItem(selItem, selItem.getParentItem().getText(), content);
